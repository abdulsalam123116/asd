...
class ReceiptController extends Controller
{
    ...
    public function updatePaymentType(Request $request)
    {
        ...
        try {
            ...
            $receiptPayment   = ReceiptPayment::find($request->id);

            if (!$receiptPayment) {
                throw new \Exception('Receipt payment not found.');
            }


            $payment_typeTemp = $request->payment_type;

            $receiptPayment->payment_type = $payment_typeTemp;
            $receiptPayment->account_no = $request->account_no;
            $receiptPayment->update();

            // Get the Cash in Hand and Cash at Bank accounts
            $cashAtHandAccount = ChartAccount::where('account_name', 'Cash in hand')->first();
            $cashAtBankAccount = ChartAccount::where('account_name', 'Cash at bank')->first();

            if ($payment_typeTemp != 'Cash') {
                // -- Convert to Cash at Bank
                SubTransaction::where('transaction_id', $receiptPayment->transaction_id)
                    ->where('account_id', $cashAtHandAccount->id)
                    ->update([
                        'account_id'   => $cashAtBankAccount->id,
                        'account_name' => $cashAtBankAccount->account_name,
                    ]);

                BankTransaction::create([
                    'transaction_id' => $receiptPayment->transaction_id,
                    'receipt_no' => $receiptPayment->receipt_no,
                    'transaction_no' => $payment_typeTemp, // Set appropriately
                    'receipt_date' => $receiptPayment->trans_date,
                    'bank_id' => 1, // Set the bank ID accordingly
                    'account_id' => $cashAtBankAccount->id,
                    'account_head' => $cashAtBankAccount->account_name,
                ...
                // -- Convert to Cash at Hand

                // Update related sub_transactions here
                SubTransaction::where('transaction_id', $receiptPayment->transaction_id)
                    ->where('account_id', $cashAtBankAccount->id)
                    ->update([
                        'account_id'   => $cashAtHandAccount->id,
                        'account_name' => $cashAtHandAccount->account_name,
                    ]);

                // Change to Cash: Remove the related BankTransaction entry
                BankTransaction::where('transaction_id', $receiptPayment->transaction_id)
                    //  ->where('account_id', $cashAtBankAccount->id)
                    ->delete();
            }
