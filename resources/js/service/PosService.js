...
export default class PosService {
    ...
    savePurchaseItem(
        ...
        formData.append(
            "pos_receipt_id",
            postObj.pos_receipt_id ? postObj.pos_receipt_id : ""
        );
        formData.append(
            "transaction_id",
            postObj.transaction_id ? postObj.transaction_id : ""
        );
    ...
    updatePaymentType(id, payment_type, account_no) {
        //SHOW LOADING
        const store = useStore();
        store.dispatch(ActionTypes.PROGRESS_BAR, true);
        const api = "/api/pos_update_payment_type";
        const formData = new FormData();
