            ...
            >
                ...
                <Column>
                    <template #body="slotProps">
                        <i
                            :class="
                                this.validateStock.includes(
                                    excelFileContent.indexOf(slotProps.data)
                                )
                                    ? 'row-invalid'
                                    : 'row-valid'
                            "
                            class="pi pi-check-circle p-p-2"
                        ></i>
                    </template>
                </Column>
...
export default class ImportStock extends Vue {
    ...
    private defaultBrandid = 0;
    private defaultBrandSectorid = 0;
        ...
        this.stockService.getItems().then((data) => {
            ...
            this.taxName1 =
                data.storeTaxes[0].show_1 == "true"
                    ? data.storeTaxes[0].tax_name_1
                    : "";
            this.taxName2 =
                data.storeTaxes[0].show_2 == "true"
                    ? data.storeTaxes[0].tax_name_2
                    : "";
            this.taxName3 =
                data.storeTaxes[0].show_3 == "true"
                    ? data.storeTaxes[0].tax_name_3
                    : "";
        });
    }
    ...
    get stockWorth() {
        ...
        this.excelFileContent.forEach((e) => {
            if (e.packSize != 0) {
                totalAmount =
                    totalAmount +
                    Number((e.quantity / e.packSize) * e.packPurchasePrice);
            }
        });
    ...
    getPackWorth(data) {
        ...
        this.excelFileContent.forEach((e) => {
            if (data.packSize != 0) {
                totalAmount = Number(
                    (data.quantity / data.packSize) * data.packPurchasePrice
                );
            }
        });
        return totalAmount.toFixed(2);
    ...
    setDefaultValues() {
        const defaultBrand = this.brand.find(
            (item) => item.option_name.toLowerCase() === "other"
        );
        const defaultBrandSector = this.brandSector.find(
            (item) => item.option_name.toLowerCase() === "other"
        );
        const defaultCategory = this.category.find(
            (item) => item.option_name.toLowerCase() === "other"
        );
        ...
        if (defaultBrandSector) {
            this.defaultBrandSectorid = defaultBrandSector.id;
        }
